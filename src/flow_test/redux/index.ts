...
export default function testPayloadReducer(
  ...
  switch (action.type) {
    case FlowTestActions.INITIALIZE_FROM_PAYLOAD:
      return {
        ...state,
        testPayload: TestPayload.initFromPayloadDef(
          action.payloadDef,
          action.name
        ),
      };
    case FlowTestActions.ADD_TEST_FIELD:
      fields = TestPayload.addTestField(
        state.testPayload.fields,
        action.newField
      );
      return {
        ...state,
    ...
    case FlowTestActions.UPDATE_TEST_FIELD:
      fields = TestPayload.updateTestField(
        state.testPayload.fields,
        action.newField
      );
      return {
    ...
    case FlowTestActions.ADD_TEST_RESULT:
      const index = state.testResults.findIndex(
        (r) => r.nodeID === action.result.nodeID
      );
      if (index === -1) {
        return {
          ...state,
